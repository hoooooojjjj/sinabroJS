...
console.log('유효한 이메일 처리:', 
    result1.fold(error => `에러: ${error}`, user => `성공: ${JSON.stringify(user)}`));
    
console.log('잘못된 이메일 처리:', 
    result2.fold(error => `에러: ${error}`, user => `성공: ${JSON.stringify(user)}`));
...
const FP = {
    ...
    identity: x => x,
    constant: x => () => x,
    tap: fn => value => { fn(value); return value; }
};

// 실무 예제: 사용자 데이터 처리 파이프라인
const rawUsers = [
    { id: 1, name: '김철수', age: 25, email: 'kim@test.com', role: 'user', active: true },
    { id: 2, name: '이영희', age: 17, email: 'lee@test.com', role: 'admin', active: false },
    { id: 3, name: '박개발', age: 30, email: 'park@test.com', role: 'user', active: true },
];
